...
class RemoveUnusedVertexGroups(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        ShowMessageBox(str(v_unused_count) + ' vertex groups were unused and have been removed.', 'Bayonetta Tool Info')
        return {'FINISHED'}
...
class MergeVertexGroupCopies(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        ShowMessageBox(str(v_merged_count) + ' vertex groups had name copies and have been merged.', 'Bayonetta Tool Info')
...
class DeleteLooseGeometrySelected(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if v_delete_count > 0:
            ShowMessageBox(str(v_delete_count) + ' vertexes have been deleted.', 'Bayonetta Tool Info')
        return {'FINISHED'}
...
class ClearSelectedBoneIDs(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        ShowMessageBox('Cleared the ID of ' + str(cleared_ids_count) + ' bones.', 'Bayonetta Tool Info')
        return {'FINISHED'}
...
class RestoreImportPose(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if active_object.type != "ARMATURE":
            ShowMessageBox('Please select an armature object.', 'Bayonetta Tool Info')
            return {'FINISHED'}
